...
5. **色と輝度の決定**
   ...
   - 未使用 LED:
     - 完全消灯でもよいが、オプションとして「弱く点灯させる」モードを用意し、なだらかな見た目を実現しても良い。
   - `brightness_max` を上限とし、全LED同時点灯時の電流が過大にならないようにする。

6. **操作系**
   - M5.BtnA:
     - 長押しで再キャリブレーション開始。
   - デバッグモード時（任意）:
     - LCD に `delta`, `level_pos`, `level_neg`, `num_pos`, `num_neg` などを表示し、挙動確認を容易にする。
